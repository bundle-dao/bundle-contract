...
import { BundleToken, BundleToken__factory, BundleLock, BundleLock__factory } from '../typechain';
...
describe('BundleLock', () => {
    ...
    beforeEach(async () => {
        ...
        // Deploy lock
        const BundleLock = (await ethers.getContractFactory('BundleLock', deployer)) as BundleLock__factory;
    ...
    context('when interacting as user', async () => {
        it('should set tiers properly', async () => {
            ...
            await expect(bundleLockAsDeployer.setLockThreshold(3, ethers.utils.parseEther('2000'))).to.be.revertedWith(
                'ERR_OUT_OF_BOUNDS'
            );
        ...
        it('should accept deposits and withdrawals correctly', async () => {
            ...
            expect(await bundleToken.balanceOf(bundleLock.address)).to.be.bignumber.and.eq(
                ethers.utils.parseEther('100')
            );
            expect(await bundleLock.getTier(await alice.getAddress())).to.be.bignumber.and.eq('2');
            expect(await bundleLock.getBundleBalance(await alice.getAddress())).to.be.bignumber.and.eq(
                ethers.utils.parseEther('100')
            );
            expect(await bundleLock.balanceOf(await alice.getAddress())).to.be.bignumber.and.eq(
                ethers.utils.parseEther('100')
            );
