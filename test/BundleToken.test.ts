...
describe("BundleToken", () => {
  ...
  context('when transferring funds', async() => {
    it('should transfer delegates during token transfers', async() => {
        ...
        expect(await bundleToken.getCurrentVotes((await carol.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('100'));
        ...
        await bundleTokenAsAlice.transferFrom((await bob.getAddress()), (await alice.getAddress()), ethers.utils.parseEther('100'));
        await bundleTokenAsAlice.delegate((await carol.getAddress()));
