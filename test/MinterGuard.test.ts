...
describe('MinterGuard', () => {
    ...
    beforeEach(async () => {
        ...
        bundleToken = await BundleToken.deploy(
        ...
        minter = await Minter.deploy(bundleToken.address, await dev.getAddress(), BUNDLE_REWARD_PER_BLOCK, 0);
        ...
        const MinterGuard = (await ethers.getContractFactory('MinterGuard', deployer)) as MinterGuard__factory;
    ...
    context("when migrate Minter's owner from Timelock to MinterGuard + Timelock", async () => {
        ...
        it('should revert when non owner to interact with MinterGuard', async () => {
            await expect(minterGuard.setRewardsPerBlock(ethers.utils.parseEther('1'))).to.be.revertedWith(
                'Ownable: caller is not the owner'
            );

            await expect(minterGuard.setBonus(1, 500, BONUS_LOCK_RATIO)).to.be.revertedWith(
                'Ownable: caller is not the owner'
            );
            ...
            await expect(minterGuard.addPool(1, stakingTokens[0].address, false)).to.be.revertedWith(
                'Ownable: caller is not the owner'
            );

            await expect(minterGuard.setPool(1, 100, false)).to.be.revertedWith('Ownable: caller is not the owner');
        });
        ...
        it('should revert when adjust param through Timelock + MinterGuard when migration has not been done', async () => {
            ...
            await timelockAsDev.queueTransaction(
                ...
                ethers.utils.defaultAbiCoder.encode(
                    ...
                    [100, stakingTokens[0].address, false]
                ),
                eta
            );
            ...
            await expect(
                timelockAsDev.executeTransaction(
                    minterGuard.address,
                    '0',
                    'addPool(uint256,address,bool)',
                    ethers.utils.defaultAbiCoder.encode(
        ...
        it('should migrate successfully', async () => {
            ...
            await timelockAsDev.queueTransaction(
                ...
                ethers.utils.defaultAbiCoder.encode(['address'], [minterGuard.address]),
                eta
            );
    ...
    context("when adjust Minter's params via MinterGuard + Timelock", async () => {
        ...
        it('should set pool on existed pool when Timelock is passed ETA', async () => {
            ...
            await timelockAsDev.queueTransaction(
                ...
                ethers.utils.defaultAbiCoder.encode(['uint256', 'uint256', 'bool'], [0, 200, false]),
                eta
            );
        ...
        it('should set bonus on Minter when Timelock is passed ETA', async () => {
            ...
            await timelockAsDev.queueTransaction(
                ...
                ethers.utils.defaultAbiCoder.encode(['uint256', 'uint256', 'uint256'], [2, block, BONUS_LOCK_RATIO]),
                eta
            );
        ...
        it('should allow to mint Bundle if mintCount <= 8m', async () => {
            ...
            await timelockAsDev.queueTransaction(
                ...
                ethers.utils.defaultAbiCoder.encode(
                    ['address', 'uint256'],
                    [await alice.getAddress(), ethers.utils.parseEther('250000')]
                ),
                eta
            );
            ...
            expect(await bundleToken.balanceOf(await alice.getAddress())).to.be.bignumber.eq(
            ...
            for (let i = 0; i < 20; i++) {
                ...
                await timelockAsDev.queueTransaction(
                    ...
                    ethers.utils.defaultAbiCoder.encode(
                        ['address', 'uint256'],
                        [await alice.getAddress(), ethers.utils.parseEther('500000')]
                    ),
                    eta
                );
            ...
            expect(await bundleToken.balanceOf(await alice.getAddress())).to.be.bignumber.eq(
                ethers.utils.parseEther('10250000')
            );
        });
        ...
        it('should revert when mintCount > 10.5m', async () => {
            ...
            await timelockAsDev.queueTransaction(
                ...
                ethers.utils.defaultAbiCoder.encode(
                    ['address', 'uint256'],
                    [await alice.getAddress(), ethers.utils.parseEther('1')]
                ),
                eta
            );
        ...
        it('should revert when amount > 500000', async () => {
            ...
            await timelockAsDev.queueTransaction(
                ...
                ethers.utils.defaultAbiCoder.encode(
                    ['address', 'uint256'],
                    [await alice.getAddress(), ethers.utils.parseEther('500001')]
                ),
                eta
            );
